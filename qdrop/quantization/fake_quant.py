...
class LSQFakeQuantize(QuantizeBase):
    ...
    def forward(self, X):
        ...
        if self.fake_quant_enabled == 1:
            ...
            # qmix
            if self.mix_prob < 1.0:
                x_prob = self.mix_prob * X + (1 - self.mix_prob) * x_orig
                return x_prob
...
class AdaRoundFakeQuantize(QuantizeBase):
    ...
    def init(self, weight_tensor: torch.Tensor, round_mode, use_ema=False):
        self.adaround = True
        self.round_mode = round_mode
        self.init_alpha(x=weight_tensor.data.clone().detach(), use_ema=use_ema)

    def init_alpha(self, x: torch.Tensor, use_ema=False):
        if self.ch_axis != -1:
